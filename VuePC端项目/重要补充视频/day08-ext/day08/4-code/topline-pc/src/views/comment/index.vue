<template>
    <el-card>
        <!-- 评论表格 -->
        <!-- el-table：表格 data: 给 table 设置数据源， table 会自动去遍历 data 中对应的数据 -->
        <el-table v-loading="loading" :data="commentList" style="width: 100%">
            <!-- commentList: [
                {title: '', comment_status: '', total_comment_coutn: '', fans_comment_coutn: ''},
                {title: '', comment_status: '', total_comment_coutn: '', fans_comment_coutn: ''},
            ] -->
            ...
            <el-table-column label="操作">
                <template slot-scope="scope">
                    <el-button @click="closeComment(scope.row)" type="text">
                        ...
                        <!-- scope.row =====> scope ====> slot-scope =====> 就是 element table 中的 teplate中的固定的属性 -->
                        <!-- 作用就是用来将当前行的数据源设置给这个 template -->
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
